...
/**
 * A case class that filters `Cell` objects whose content is equal a specified value.
 *
 * This class extends the `ValueFilter` abstract class, specializing it to filter `IntCell`
 * instances where the integer content is equal the provided value.
 *
 * @param value The threshold value for filtering.
 */
case class EqualFilter(override val value: Int) extends ValueFilter(value) {

    /**
     * Filters the input to determine if it is an `IntCell` with content equal the threshold value.
     *
    ...
    override def doFilter(input: Cell[_]): Boolean = input match {
        case cell: IntCell => cell.cellContent == value // Check if the content of the `IntCell` is equal the threshold.
