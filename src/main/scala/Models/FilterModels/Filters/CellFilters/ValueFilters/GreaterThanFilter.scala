...
/**
 * A case class that filters `Cell` objects whose content is greater than a specified value.
 *
 ...
 */
case class GreaterThanFilter(override val value: Int) extends ValueFilter(value) {

    /**
     * Filters the input to determine if it is an `IntCell` with content greater than the threshold value.
     *
     * @param input The `Cell[_]` instance to be evaluated.
     * @return `true` if the input is an `IntCell` with content greater than `value`,
     *         `false` otherwise.
     */
